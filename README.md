...
Verify the install — open PowerShell and run:
...
This installs Express, Multer, the Anthropic SDK, and other packages. Takes about 30 seconds.
...
Do this every time you want to use the wine app.
...
1. Click **Scan** or the upload zone
2. Upload a photo of one or more wine bottles (JPEG, PNG, WEBP — max 20 MB)
3. Claude Vision reads the labels and fills in all the details automatically
4. Review the results and click **Add to Cellar** to save

### iPhone Photos (HEIC format)

iPhones save photos as HEIC by default, which cannot be analyzed directly. Two options:
...
Your `.env` file is missing or has the wrong key. Check the file contents and
...
Check the PowerShell window for error messages after scanning. Usually means
the API key is missing, expired, or has no remaining credits.

**Backup your cellar data**

Your data lives in `wine_inventory.json`. Copy this file somewhere safe
periodically, or export to CSV using the link above.
...
| Export to CSV | http://localhost:3000/api/export/csv |
